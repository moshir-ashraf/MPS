...
void MainMap::showpath(){
    ...
    for(int j=0; j<graph.path.size();j++){
        ...
        distance+= graph.path[j]->shortestsl(graph.path[j+1]);
    }
...
void MainMap::on_AddFav_clicked()
{if(graph.start!=NULL){
    bool favFound = false;
...
void MainMap::on_Change_clicked()
{
...
void MainMap::setUser(User user)
{
    this->users = &user;
...
void MainMap::on_Favs_currentIndexChanged(int index)
{
    reset();
    if (ui->rs->isChecked()){
